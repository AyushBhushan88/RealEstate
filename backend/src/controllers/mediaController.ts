...
export const uploadPropertyMedia = async (req: Request, res: Response) => {
  try {
    ...
    if (property.agentId !== (req as any).user.userId && (req as any).user.role !== 'ADMIN') {
    ...
    const mediaData = files.map((file, index) => ({
      propertyId,
      url: file.path,
      isMain: index === 0 && (property as any).mediaCount === 0, // Simplified logic for main image
      order: index,
      type: 'IMAGE'
    }));
